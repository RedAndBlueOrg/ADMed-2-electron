...
| 2026-09-16 | 오프라인/DNS 미준비 부팅(auto-launch 가 네트워크보다 먼저 뜸) 시 **HLS 콘텐츠 전멸 + 재생 루프 무증상 사망** — 캐시에 패키지가 멀쩡히 있어도 재생 안 됨 (코드 리딩 + 하네스 실증, 현장 관측 보고는 없음) | `hls.js` 를 unpkg CDN 에서 로드하고 있었다(`index.html`, CSP `script-src` 에 unpkg 허용). 스크립트가 안 오면 `window.Hls` 가 undefined 이고 Chromium 은 네이티브 HLS 미지원 → `media.js` 의 `HLS playback not supported` 분기가 **동기** `callPlayNext()` 호출 → 사이클 전체가 `loadPlaylist` 의 실행 스택 안에서 붕괴 → 끝에서 부르는 `loadPlaylist({fromCycle})` 이 **아직 `finally` 전이라 `playlistLoading === true` 인 바깥 호출**의 재진입 가드에 막혀 조용히 return, `retryTimer` 조차 안 걸림. tester 하네스 실증: `prepareCount` 1 고정(사망) → 수정 후 정상 순환. | ① `hls.js@1.6.15` 를 npm 의존성으로 옮겨 `./node_modules/hls.js/dist/hls.min.js` 로컬 로드 + CSP `script-src 'self'` 로 축소(외부 스크립트 의존 0건 확인, 부팅마다 서드파티 CDN 코드를 받아 실행하던 공급망 표면도 제거) ② `callPlayNext()` 를 `setTimeout(..., 0)` 으로 감싸 항상 새 스택에서 시작 — 동기 연쇄 자체를 없애 재진입 데드락 해소 | impact-map 2건 등재("`setTimeout` ↔ `playlistLoading` 재진입 가드", "hls.js script ↔ dependencies ↔ `files`/`asar`"). **배포 전 `npm run dist:local` → `npx asar list ... \| grep hls.min.js` 로 번들 포함 확인 필수** — 이 실패는 로컬 `npm start` 로는 절대 안 잡히고 패키징 산출물에서만 드러난다. 두 수정이 서로의 안전망: asar 포함이 실패해도 ②덕에 루프는 안 죽고 HLS 항목만 skip 된다. | 1 | ⚠ |
