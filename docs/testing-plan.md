...
모듈 로딩 테스트
---------------
- **Main 모듈**: `npm start` 후 콘솔에 모듈 로딩 오류(require 실패, undefined 접근)가 없는지 확인합니다. 특히 `config.js`의 lazy getter와 `updater.js`의 lazy require가 정상 동작하는지 확인합니다.
- **Renderer 모듈**: DevTools 콘솔에 ES Module import 오류(`Failed to load module script`)가 없는지 확인합니다. `<script type="module">`이 정상 로드되고 DOM 참조가 모두 유효한지 확인합니다.
...
- **다운로드 진행률**: 콘텐츠 다운로드 시 `overlays.js`의 다운로드 오버레이가 표시되고 진행률이 업데이트되는지 확인합니다.
...
- **대기/클리닉 연동**: `waitingInfo === 'Y'` 시 REST 응답이 카드에 반영되고, WebSocket 이벤트 수신 시 리스트/호출 팝업/음성이 즉시 업데이트되는지 확인합니다.
- **호출 알림**: 연속 호출 이벤트를 전송해 팝업 큐가 순차 처리되는지, 동영상 재생이 일시 중지/재개되는지 확인합니다.
- **랜딩 화면**: 시리얼 미설정 시 `LANDING_URL`이 iframe으로 표시되고, 시리얼 설정 후 플레이어로 돌아오는지 확인합니다.
...
업데이트 동기화 테스트
-------------------
- 콘텐츠 다운로드 중 업데이트가 완료된 상황을 시뮬레이션해 `quitAndInstall`이 지연되는지 확인합니다.
- 콘텐츠 동기화 완료 후 자동으로 업데이트가 설치되는지 확인합니다.

회복·성능 확인
-------------
- **네트워크 단절**: 재생 중 네트워크를 끊었다가 복구해도 캐시된 자산은 계속 재생되고, 연결 복원 후 공지/대기/날씨가 자동 갱신되는지 확인합니다.
- **온라인 복구**: `playlist.js`의 `startOnlineCheck()`가 네트워크 복귀를 감지하고 `attemptRecovery()`를 호출하는지 확인합니다.
